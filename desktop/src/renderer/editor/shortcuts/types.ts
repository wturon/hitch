...
// MarkdownTransformers.ts — the exported transformer TYPES). We copy them here,
// rather than importing from @lexical/markdown, because that package statically
// imports @lexical/code (sideEffects:true → prismjs), which retains prismjs in
// the bundle even when we only touch types/transformers that never construct a
// CodeNode. See ./registerMarkdownShortcuts.ts for the engine and the
// bundle-escape rationale. Types only — verbatim from the installed 0.35.0.
import type {
  ElementNode,
  Klass,
  LexicalNode,
