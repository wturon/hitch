...
// NOTE (docs/chat-tracking-redesign.md §8 "Cut"): cmux calls used to ALSO emit a
// structured per-chat trace into the local sqlite store's `cmux_trace` table,
// through an ambient AsyncLocalStorage chat context. The store is gone, and with
// it the table, the ambient context and the sink. The human log below is what
// remains — and when the Chat Inspector (§9) wants the structured version, it
// gets it from the server, not from a second local database.
...
export async function startCommand(spec: StartCommandSpec): Promise<OpenResult> {
  const memo = recentSpawns.get(spec.taskKey);
  if (memo && Date.now() - memo.at < SPAWN_GRACE_MS) {
    try {
      await cmux(["select-workspace", "--workspace", memo.workspace]);
